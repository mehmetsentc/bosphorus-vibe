  ...
  try {
    ...
    while (marked < maxMark) {
      ...
      for (const doc of snap.docs) {
        scanned += 1;
        const data = doc.data();
        if (!postNeedsThumbnailRegen(data)) continue;

        const status = data.videoThumbnailStatus as string | undefined;
        ...
        batch.update(doc.ref, {
          ...
          videoThumbnailUpdatedAt: new Date(),
        });
        marked += 1;
        batchWrites += 1;
    ...
    await writeAuditLog("admin_thumbnail_enqueue", adminUid, {
      ip,
      scanned,
      marked,
      alreadyQueued,
